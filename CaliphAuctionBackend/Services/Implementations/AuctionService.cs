...
/// </summary>
[AddScoped]
public class AuctionService(CaliphDbContext dbContext, IConfiguration configuration, IHubContext<AuctionHub, IAuctionClient> hubContext, IHttpContextAccessor httpContextAccessor) : IAuctionService {
	private readonly IConfiguration _configuration = configuration;
	private readonly CaliphDbContext _db = dbContext;
	...
	public async Task<AuctionItemDetailDto> GetDetailAsync(int id) {
		...
		if (item is null) {
			throw new CaliphException("Auction item not found", null, HttpStatusCode.NotFound);
		}
	...
	public async Task PlaceBidAsync(int userId, PlaceBidRequest request, string ipAddress) {
		...
		if (item.Status != AuctionStatus.Active) {
			throw new ValidationCaliphException("Auction is not active.");
		}

		// すでに現在の最高入札者が自分の場合、入札不可
		if (item.CurrentHighestBidUserId.HasValue && item.CurrentHighestBidUserId.Value == userId) {
			throw new ValidationCaliphException("You are already the highest bidder.");
		}
		...
		if (request.BidAmount != expectedNextAmount) {
			throw new ValidationCaliphException($"Invalid bid amount. The next bid must be exactly {expectedNextAmount}.");
		}
		...
		if (nullableMinimumEndTimeSeconds is not { } minimumEndTimeSeconds) {
			throw new ConfigurationCaliphException("Auction:MinimumEndTimeSeconds configuration is missing.");
		}
	...
	private async Task<int> ConsumeBidPointsAsync(User user, Bid bid, int perBidCost) {
		if (user.PointBalance < perBidCost) {
			throw new ValidationCaliphException("Insufficient points.");
		}
