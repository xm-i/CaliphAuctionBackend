...
[AddScoped]
public class PointService(CaliphDbContext db, IConfiguration config) : IPointService {
	...
	public async Task<RedeemDepositResponse> PurchaseAsync(int userId, RedeemDepositRequest request) {
		if (string.IsNullOrWhiteSpace(request.DepositToken)) {
			throw new ValidationCaliphException("DepositToken required");
		}

		var pointPlan = await this._db.PointPlans.FirstOrDefaultAsync(p => p.Id == request.PointPlanId);
		if (pointPlan == null) {
			throw new ValidationCaliphException("Invalid point plan");
		}

		var extKey = this._config["ExternalPaymentJwt:Key"] ?? throw new ConfigurationCaliphException("ExternalPaymentJwt:Key configuration is missing.");
		var extIssuer = this._config["ExternalPaymentJwt:Issuer"] ?? throw new ConfigurationCaliphException("ExternalPaymentJwt:Issuer configuration is missing.");
